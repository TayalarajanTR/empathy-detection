import codecs, csv, re, numpy as np, argparse
...
for row in csv_reader:
    ...
    enc_pad = tokenizer(
        response, add_special_tokens=True, padding="max_length", truncation=True, max_length=64, return_tensors=None
    )
    enc_trim = tokenizer(
        response, add_special_tokens=True, truncation=True, max_length=64, return_tensors=None
    )

    # Reconstruct tokenized text so the old logic works
    response_tokenized = tokenizer.decode(enc_pad["input_ids"], clean_up_tokenization_spaces=False)
    response_tokenized_non_padded = tokenizer.decode(enc_trim["input_ids"], clean_up_tokenization_spaces=False)
    ...
    for r in rationales:
        ...
        try:
            ...
            curr_match = list(set(curr_match)); curr_match.sort()
